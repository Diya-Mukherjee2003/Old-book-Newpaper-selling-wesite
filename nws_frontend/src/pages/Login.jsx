...
const Login = () => {
  ...
  const handleSubmit=async(e)=>{
    e.preventDefault()
    try{
      const api=await axios.post(`http://localhost:5000/api/users/login`,{
        email,
        password
      },{
        headers:{
  ...
  return (
    <div className='flex items-center justify-center min-h-screen'>
       <div className='w-full max-w-sm bg-amber-200 shadow-lg rounded-lg p-8 '>
        <h1 className="text-2xl font-bold mb-4 text-center text-yellow-900">Login</h1>
          ...
          <button className='bg-amber-600 hover:bg-amber-700 text-white border p-2 rounded-xl' type='submit'>Login</button>
        </form>
      </div>
    </div>
  )
